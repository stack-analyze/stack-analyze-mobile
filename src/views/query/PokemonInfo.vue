...
<template>
  <ion-page>
    ...
    <ion-content>
      <ion-item>
        <ion-input autocomplete="off" color="secondary" :clear-input="true" label="enter a pokemon name or id:"
          label-placement="floating" type="text" v-model.number="pokemon" />
      </ion-item>

      <ion-grid>
        <stack-buttons init-btn-name="get pokemon" :init-validate="false" @init-function="pokemonInfo"
          clear-btn-name="clear pokemon" :clear-validate="isEmptyPokeInfo" @clear-function="clearPokemonInfo" />
      </ion-grid>

      <ion-card mode="ios">
        <swiper-container :modules="modules" :loop="true">
          <swiper-slide v-for="(sprite, i) in pokesprites" :key="i">
            <ion-img :src="sprite" class="pokemon" alt="default sprite" />
          </swiper-slide>
        </swiper-container>

        <ion-card-header class="ion-align-items-center" mode="md">
          <ion-card-title>
            #{{ pokemonData?.id ?? 0 }} -
          ...
          <ion-item lines="none">
            <ion-chip v-for="type of pokemonData?.types" :key="type.slot" :class="[type.type.name, 'info']">
              {{ type.type.name }}
            </ion-chip>
          </ion-item>
        </ion-card-header>
        ...
        <ion-card-content class="ion-text-center" mode="md">
          <ion-chip v-for="stats of pokemonStats" :key="stats.name" :outline="true" :color="stats.color" class="info">
