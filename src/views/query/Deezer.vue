...
<template>
	<ion-page>
		...
		<ion-content>
			<ion-item>
				<ion-input autocomplete="off" color="secondary" :clear-input="true" label="enter a search"
					label-placement="floating" type="text" v-model="query" />
			</ion-item>

			<ion-grid>
				<stack-buttons init-btn-name="start search" :init-validate="false" @init-function="getAlbums"
					clear-btn-name="clear search" :clear-validate="isAlbumsEmpty" @clear-function="clearAlbums" />

				<ion-row :fixed="true">
					<ion-col size-lg="3" size-md="4" size-sm="6" size="12" v-for="album of albums">
						<ion-card>
							<ion-card-header>
								<ion-img :src="album.cover_xl" :alt="album.id.toString()" />
